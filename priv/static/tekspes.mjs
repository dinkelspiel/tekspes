...
var getAttributeFromEventTarget = (event2, attribute2) => {
  if (Object.keys(event2.target.dataset).includes(attribute2)) {
    return new Ok(event2.target.dataset[attribute2]);
  } else {
    return new Error(void 0);
  }
};
...
function do_for(loop$callback, loop$amount, loop$acc) {
  while (true) {
    ...
    if (amount === 0) {
      ...
      loop$acc = prepend(callback(amount), acc);
    }
  }
}
function for$(amount, callback) {
...
function is_collision_toggled(collisions, pos) {
  let $ = get(collisions, pos);
  if ($.isOk()) {
    let a = $[0];
    return a;
  } else {
...
function view(model) {
  return div(
    ...
    toList([
      div(
        toList([class$("w-[2048px]")]),
        toList([
          img(toList([src("/priv/static/skolan.png")])),
          div(
            toList([
              class$(
                "absolute top-0 grid grid-cols-[repeat(128,16px)]"
              )
            ]),
            for$(
              128 * 35,
              (idx) => {
                let x = remainderInt(idx, 128);
                let y = (() => {
                  let _pipe = floor2(
                    divideFloat(to_float(idx), 128)
                  );
                  return round2(_pipe);
                })();
                return div(
